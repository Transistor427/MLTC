...
| `retract` | Ретракт перед отъездом, мм |
| `retract_speed` | Скорость ретракта, мм/с |
| `unretract_extra` | Дополнительная подача при возврате, мм |
| `unretract_speed` | Скорость возврата филамента, мм/с |

При нехватке времени слоя: ретракт → подъём Z → парковка XY → ожидание → возврат XY → опускание Z → unretract.
